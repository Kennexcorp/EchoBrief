...
| **Privacy** | In the default configuration, no network calls leave localhost. Audio files and transcripts are processed in a temp directory and deleted on session end. **Sole exception (F8, opt-in):** if the user supplies an `ELEVENLABS_API_KEY`, brief text is sent to the ElevenLabs API for speech synthesis. Absent that key the feature is inert, so a default install remains fully local. Audio and transcripts are never transmitted; only the generated brief text is. |
