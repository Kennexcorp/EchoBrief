...
| F8 | *(added post-MVP, 2026-07-20)* User can optionally hear the brief read aloud. Off by default; activates only when an `ELEVENLABS_API_KEY` is set. This is the single exception to the localhost-only privacy guardrail, and it is opt-in by construction. |
| F9 | *(added post-MVP, 2026-07-28)* Transcript segments are labelled by speaker, and those labels are passed to the LLM so action-item ownership is grounded in the audio. Optional: needs the diarization extra and a Hugging Face token. Absent either, the pipeline degrades to an unlabelled transcript with a message naming the exact fix. |
...
| **Privacy** | In the default configuration, no network calls leave localhost. Audio files and transcripts are processed in a temp directory and deleted on session end. **Sole exception (F8, opt-in):** if the user supplies an `ELEVENLABS_API_KEY`, brief text is sent to the ElevenLabs API for speech synthesis. Absent that key the feature is inert, so a default install remains fully local. Audio and transcripts are never transmitted; only the generated brief text is. **Weight downloads (F9):** with diarization enabled, pyannote fetches model weights from Hugging Face on first run, and the `HUGGINGFACE_TOKEN` is transmitted there at download time. This is the same class of access the Whisper weights already use, and inference itself is fully local: no audio or transcript leaves the machine. |
...
| **Packaging & distribution** | `uv` + host-installed Ollama (primary path); Docker + docker-compose (secondary path) | pip + venv, Docker-only, uv-only | Deliberate dual-path decision matched to the audience. **Primary:** non-technical students get the lightest install — `uv sync` plus the one-click Ollama installer. uv beats pip + venv on exactly this audience's failure modes: no separate Python install (uv provisions it), no venv-activation step (the classic Windows tripwire), and `uv.lock` makes the light path reproducible, not just the Docker path. Docker Desktop would add friction for exactly the users we're targeting. **Secondary:** a Dockerfile (app) + `docker-compose.yml` (app + `ollama/ollama` service, shared network, named volume for model weights) gives reviewers and power users a one-command reproducible stand-up of the full architecture, eliminating the ffmpeg/CTranslate2 "works on my machine" class of issues entirely. Trade-offs accepted: image stays lean by mounting a Whisper model cache volume instead of baking multi-GB weights into the image; GPU passthrough (NVIDIA Container Toolkit) documented but not required. **Post-build update (2026-07-19):** once a public, multi-arch image is published to GHCR, `docker pull` + `docker run` (against host Ollama) becomes the fastest *try-it* path and the README leads with it; `uv sync` remains the recommended path for **developing** the project. The design-time "uv-first" reasoning was about installing from source, which had no published image to compete with. |
| **Speech synthesis (F8, added 2026-07-20)** | ElevenLabs REST API via stdlib `urllib`, opt-in behind `ELEVENLABS_API_KEY` | Local TTS (Piper, Coqui, `pyttsx3`/system voices), no voice feature at all | Accepting a cloud dependency here was the deliberate trade-off, and it is the only place we make it. Local TTS would have preserved the localhost-only guarantee absolutely, but Piper and Coqui each add a multi-hundred-MB model plus a real dependency and platform-matrix cost, and system voices sound poor enough to undercut the feature's point. Because the feature is a leaf (it consumes a finished `Brief` and produces bytes), it can be gated cleanly: no key means no code path, so the default install keeps the privacy guarantee intact and the NFR is amended, not abandoned. Cost containment: stdlib `urllib` means zero new dependencies, and only generated brief text crosses the wire, never audio or transcripts. `brief_to_script` is a pure function so the interesting logic stays unit-tested offline; the HTTP call sits behind an injected `SpeechClient` protocol, consistent with the constructor-injection seam used everywhere else. |

| **Speaker diarization (F9, added 2026-07-28)** | `pyannote.audio` running alongside faster-whisper, merged by timestamp overlap, behind an optional extra | WhisperX (bundles diarization with a re-implemented transcription path), no diarization at all | WhisperX would have replaced our transcription layer wholesale to gain diarization, discarding a tested `TranscriptionService` and the published faster-whisper benchmarks we selected on. Running pyannote alongside keeps that layer untouched: the two outputs are joined by a pure `assign_speakers` function, so the interesting logic (max-overlap wins, ties to the earlier turn, no overlap leaves `speaker=None`) is unit-tested without audio, a GPU, or a token. Cost containment: pyannote pulls torch, which is far too heavy for the default install, so it sits behind `uv sync --extra diarization` and the import happens inside the factory rather than at module scope. That is what lets the pipeline degrade to an unlabelled transcript instead of failing to start. Model choice: `speaker-diarization-community-1` over `speaker-diarization-3.1` because 3.1 ships no weights and pulls from the also-gated `pyannote/segmentation-3.0`, so it costs the user two terms acceptances instead of one. The price is attribution: community-1 is CC-BY-4.0, where 3.1 is MIT. |
...
| Gated-model friction blocks diarization (F9) | High (first-run) | Low | The feature is optional by construction: without the extra or the token it degrades to an unlabelled transcript rather than failing. Each failure path raises `DiarizationError` with a message naming the exact fix (`uv sync --extra diarization`, or the terms-acceptance URL), and `apply_diarization` never raises, so the brief is always produced. |
